    ...
    async def checker(msg: Message):
        if _check_creds():
            ...
            await msg.edit(
                ...
                " For more info see` [this post](https://t.me/UnofficialPluginsHelp/)."
            )
    return checker
...
async def _lastfm(msg: Message):
    ...
    if msg.flags and '-on' in msg.flags:
        ...
        await msg.edit(out, disable_web_page_preview=False)
    ...
    while NOW_PLAYING[0] is True and await _get_now_playing(user) is not None:
        ...
        if NOW_PLAYING[1] != song.get_name():
            ...
            for chat_id in CHAT_IDS:
                out = f"{msg.from_user.mention} __is currently Listening to:__\n\n"
                out += get_track_info(song)
                if userge.has_bot:
                    try:
                        await userge.bot.send_message(chat_id, out)
                    except (ChatWriteForbidden, ChannelPrivate, ChatIdInvalid):
                        await userge.send_message(chat_id, out)
...
def get_track_info(track: pylast.Track) -> str:
